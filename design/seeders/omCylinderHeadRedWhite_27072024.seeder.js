...
const flagCreatedBy = "SEEDER CylinderHead 27072024"
...
const main = async () => {
  ...
  try {
    ...
    const lineQuery = await db.query(
      `select * from tb_m_lines where line_nm in ('Cylinder Head') and line_id = 1 and deleted_dt is null limit 1`
    )
    const lineRow = lineQuery.rows[0]

    const json = JSON.parse(
      await readFile(
        path.resolve(__dirname, "../json/omCylinderHeadRedWhite.json"),
        "utf8"
      )
    )
    for (let i = 0; i < json.length; i++) {
      ...
      //#region item check func
      const itemCheckGen = async (group_id) => {
        //#region item check
        ...
        if (itemCheckExisting.rowCount == 0) {
          ...
          const newlyInsertedItemCheck = await db.query(
            newlyInsertedItemCheckSql
          )
      ...
      let isRedAndWhite = false
      if (groupName.toLowerCase() == "w") {
        groupName = "WHITE"
      } else if (groupName.toLowerCase() == "r") {
        groupName = "RED"
      } else if (
        groupName.toLowerCase().includes("red") &&
        groupName.toLowerCase().includes("white")
      ) {
        isRedAndWhite = true
      }

      const groupSql = `select * from ${table.tb_m_groups} where group_nm ${
        isRedAndWhite ? `in ('RED', 'WHITE')` : `= '${groupName}'`
      }`

      let groupRow = await db.query(groupSql)
      if (isRedAndWhite) {
        for (let gIndex = 0; gIndex < groupRow.rows.length; gIndex++) {
            await itemCheckGen(groupRow.rows[gIndex].group_id)   
        }
      } else {
        ...
        await itemCheckGen(groupRow.group_id)
      }
      //#endregion
    }
