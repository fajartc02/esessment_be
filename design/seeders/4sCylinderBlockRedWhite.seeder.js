...
const clearRows = async (db = databasePool) => {
    ...
    await db.query(`DELETE FROM ${table.tb_m_4s_item_check_kanbans} WHERE created_by = 'SEEDER Cylinder Block 23052024'`)
    ...
    /* if (moment(lastItemCheck.rows[0].created_date).format('YYYY-MM-DD') == moment().format('YYYY-MM-DD') || lastItemCheck.rows[0].created_by == 'SEEDER Cylinder Block 23052024')
    {
    ...
    await db.query(`DELETE FROM ${table.tb_m_kanbans} WHERE created_by = 'SEEDER Cylinder Block 23052024'`)
    ...
    /* if (moment(lastKanban.rows[0].created_date).format('YYYY-MM-DD') == moment().format('YYYY-MM-DD') || lastKanban.rows[0].created_by == 'SEEDER Cylinder Block 23052024')
    {
        await db.query(`ALTER TABLE ${table.tb_m_kanbans} ALTER COLUMN kanban_id RESTART WITH ${lastKanban.rows[0].kanban_id + 1}`)
    } */

    await db.query(`DELETE FROM ${table.tb_m_zones} WHERE created_by = 'SEEDER Cylinder Block 23052024'`)
    ...
    /* if (moment(lastZone.rows[0].created_date).format('YYYY-MM-DD') == moment().format('YYYY-MM-DD') || lastZone.rows[0].created_by == 'SEEDER Cylinder Block 23052024')
    {
        await db.query(`ALTER TABLE ${table.tb_m_zones} ALTER COLUMN zone_id RESTART WITH ${lastZone.rows[0].zone_id + 1}`)
    } */

    await db.query(`DELETE FROM ${table.tb_m_freqs} WHERE created_by = 'SEEDER Cylinder Block 23052024'`)
    ...
    /* if (moment(lastFreq.rows[0].created_date).format('YYYY-MM-DD') == moment().format('YYYY-MM-DD') || lastFreq.rows[0].created_by == 'SEEDER Cylinder Block 23052024')
    {
...
const main = async () => {
    try
    {
        ...
        const lineQuery = await db.query(`select * from tb_m_lines where line_nm in ('Cylinder Block') limit 1`)
        const lineRow = lineQuery.rows[0]

        const json = JSON.parse(await readFile(path.resolve(__dirname, '../json/4SCylInderBlockRedWhite.json'), "utf8"));
        for (let i = 0; i < json.length; i++)
        {
            ...
            const zoneSql = `
                    ...
                    where 
                        ...
                        and zone_nm = 'Zone ${json[i].Zona.replace('ZONE ', '').replace(/("|')/g, "")}' 
                `
            ...
            else
            {
                ...
                const newZoneSql = `
                    ...
                    values 
                        ('${uuid()}', '${lineRow.line_id}', 'Zone ${json[i].Zona.replace('ZONE ', '').replace(/("|')/g, "")}', 'SEEDER Cylinder Block 23052024', 'SEEDER Cylinder Block 23052024') 
                    returning *
                `
            ...
            else
            {
                ...
                const newFreqSql = `
                    ...
                    values
                        ('${uuid()}', '${freqNm}', '${precitionVal}', 'SEEDER Cylinder Block 23052024', 'SEEDER Cylinder Block 23052024')
                    returning *
                `
            ...
            //#region fetch group
            const groupSql = `select * from ${table.tb_m_groups} where group_nm = '${json[i].Group.replace(/("|')/g, "")}'`
            ...
            const kanbanSql = `
                ...
                where
                    zone_id = '${zoneId}'
                    and freq_id = '${freqId}'
                    and kanban_no = '${json[i].no_kanban.replace(/("|')/g, "")}'
                    and area_nm = '${json[i].Area.replace(/("|')/g, "")}'
            `
            ...
            else
            {
                ...
                const newKanbanSql = `
                    ...
                    values
                        ('${uuid()}', '${zoneId}', '${freqId}', '${json[i].no_kanban.replace(/("|')/g, "")}', '${json[i].Area.replace(/("|')/g, "")}', '${groupRow.group_id}', 'SEEDER Cylinder Block 23052024', 'SEEDER Cylinder Block 23052024')
                    returning *
                `
            ...
            where 
                kanban_id = '${kanbanId}' 
                and item_check_nm = '${json[i]['Item Check Kanban'].replace(/("|')/g, "")}'
                and control_point = '${json[i]['Control Point'].trimStart().replace(/("|')/g, "")}'
                and method = '${json[i]['Metode 4S'].replace(/("|')/g, "")}'
        `
            ...
            if (itemCheckQuery.rowCount == 0)
            {
                ...
                const newItemCheckSql = `
                    ...
                    values
                        ('${uuid()}', '${kanbanId}', '${json[i]['Item Check Kanban'].replace(/("|')/g, "")}', '${json[i]['Time']}', '${json[i]['Metode 4S'].replace(/("|')/g, "")}', '${json[i]['Control Point'].trimStart().replace(/("|')/g, "")}',  'SEEDER Cylinder Block 23052024', 'SEEDER Cylinder Block 23052024')
                    returning *
                `
