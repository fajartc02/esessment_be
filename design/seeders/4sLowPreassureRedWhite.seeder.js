...
const clearRows = async (db = databasePool) => {
    ...
    {
        await db.query(`DELETE FROM ${table.tb_r_4s_schedule_sign_checkers} WHERE created_by = 'SEEDER LowPreassure 12062024'`)
        ...
        await db.query(`DELETE FROM ${table.tb_r_4s_sub_schedules} WHERE created_by = 'SEEDER LowPreassure 12062024'`)
        ...
        await db.query(`DELETE FROM ${table.tb_r_4s_main_schedules} WHERE created_by = 'SEEDER LowPreassure 12062024'`)
    ...
    {
        await db.query(`DELETE FROM ${table.tb_m_4s_item_check_kanbans} WHERE created_by = 'SEEDER LowPreassure 12062024'`)
        ...
        await db.query(`DELETE FROM ${table.tb_m_kanbans} WHERE created_by = 'SEEDER LowPreassure 12062024'`)
        ...
        await db.query(`DELETE FROM ${table.tb_m_zones} WHERE created_by = 'SEEDER LowPreassure 12062024'`)
        ...
        await db.query(`DELETE FROM ${table.tb_m_freqs} WHERE created_by = 'SEEDER LowPreassure 12062024'`)
...
const main = async () => {
    try
    {
        ...
        const lineQuery = await db.query(`select * from tb_m_lines where line_nm in ('Low Pressure') limit 1`)
        const lineRow = lineQuery.rows[0]

        const json = JSON.parse(await readFile(path.resolve(__dirname, '../json/4sLowPreassureRedWhite.json'), "utf8"));
        for (let i = 0; i < json.length; i++)
        {
            ...
            else
            {
                ...
                const newZoneSql = `
                    ...
                    values 
                        ('${uuid()}', '${lineRow.line_id}', 'Zone ${zoneNm}', 'SEEDER LowPreassure 12062024', 'SEEDER LowPreassure 12062024') 
                    returning *
                `
            ...
            else
            {
                ...
                const newFreqSql = `
                    ...
                    values
                        ('${uuid()}', '${freqNm}', '${precitionVal}', 'SEEDER LowPreassure 12062024', 'SEEDER LowPreassure 12062024')
                    returning *
                `
            ...
            else
            {
                ...
                const newKanbanSql = `
                    ...
                    values
                        ('${uuid()}', '${zoneId}', '${freqId}', '${noKanban}', '${json[i].Area.replace(/("|')/g, "").trim()}', '${groupRow.group_id}', 'SEEDER LowPreassure 12062024', 'SEEDER LowPreassure 12062024')
                    returning *
                `
