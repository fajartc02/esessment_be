...
const clearRows = async (db = databasePool) => {
    ...
    await db.query(`DELETE FROM ${table.tb_m_4s_item_check_kanbans} WHERE created_by = 'SEEDER WHITE'`)
    ...
    /* if (moment(lastItemCheck.rows[0].created_date).format('YYYY-MM-DD') == moment().format('YYYY-MM-DD') || lastItemCheck.rows[0].created_by == 'SEEDER WHITE')
    {
    ...
    await db.query(`DELETE FROM ${table.tb_m_kanbans} WHERE created_by = 'SEEDER WHITE'`)
    ...
    /* if (moment(lastKanban.rows[0].created_date).format('YYYY-MM-DD') == moment().format('YYYY-MM-DD') || lastKanban.rows[0].created_by == 'SEEDER WHITE')
    {
        await db.query(`ALTER TABLE ${table.tb_m_kanbans} ALTER COLUMN kanban_id RESTART WITH ${lastKanban.rows[0].kanban_id + 1}`)
    } */

    await db.query(`DELETE FROM ${table.tb_m_zones} WHERE created_by = 'SEEDER WHITE'`)
    ...
    /* if (moment(lastZone.rows[0].created_date).format('YYYY-MM-DD') == moment().format('YYYY-MM-DD') || lastZone.rows[0].created_by == 'SEEDER WHITE')
    {
        await db.query(`ALTER TABLE ${table.tb_m_zones} ALTER COLUMN zone_id RESTART WITH ${lastZone.rows[0].zone_id + 1}`)
    } */

    await db.query(`DELETE FROM ${table.tb_m_freqs} WHERE created_by = 'SEEDER WHITE'`)
    ...
    /* if (moment(lastFreq.rows[0].created_date).format('YYYY-MM-DD') == moment().format('YYYY-MM-DD') || lastFreq.rows[0].created_by == 'SEEDER WHITE')
    {
...
const main = async () => {
    try
    {
        ...
        const json = JSON.parse(await readFile(path.resolve(__dirname, '../json/4sMainLineWhite.json'), "utf8"));
        for (let i = 0; i < json.length; i++)
        {
            ...
            else
            {
                ...
                const newZoneSql = `
                    ...
                    values 
                        ('${uuid()}', '${assyLineRow.line_id}', 'Zone ${json[i].Zona}', 'SEEDER WHITE', 'SEEDER WHITE') 
                    returning *
                `
            ...
            else
            {
                ...
                const newFreqSql = `
                    ...
                    values
                        ('${uuid()}', '${freqNm}', '${precitionVal}', 'SEEDER WHITE', 'SEEDER WHITE')
                    returning *
                `
            ...
            else
            {
                ...
                const newKanbanSql = `
                    ...
                    values
                        ('${uuid()}', '${zoneId}', '${freqId}', '${json[i].no_kanban}', '${json[i].Area}', '${groupRow.group_id}', 'SEEDER WHITE', 'SEEDER WHITE')
                    returning *
                `
        ...
        `
            ...
            if (itemCheckQuery.rowCount == 0)
            {
                ...
                const newItemCheckSql = `
                    ...
                    values
                        ('${uuid()}', '${kanbanId}', '${json[i]['Item Check Kanban']}', '${json[i]['Time']}', '${json[i]['Metode 4S']}', '${json[i]['Control Point'].trimStart()}',  'SEEDER WHITE', 'SEEDER WHITE')
                    returning *
                `
