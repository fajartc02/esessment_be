...
const { createNewKanbanSingleLineSchedule } = require("../../services/om.services")
...
const clearRows = async (db = database) => {
    ...
    //#region clear transaction with flag
    {
        ...
        const lastTransSignChecker = await db.query(`SELECT *, date(created_dt) as created_date FROM ${table.tb_r_om_schedule_sign_checkers} ORDER BY om_sign_checker_id DESC LIMIT 1`)
        await db.query(`ALTER TABLE ${table.tb_r_om_schedule_sign_checkers} ALTER COLUMN om_sign_checker_id RESTART WITH ${(lastTransSignChecker.rows[0]?.om_sign_checker_id ?? 0) + 1}`)

        await db.query(`DELETE FROM ${table.tb_r_om_sub_schedules} WHERE created_by = '${flagCreatedBy}'`)
        const lastTransSubSchedule = await db.query(`SELECT *, date(created_dt) as created_date FROM ${table.tb_r_om_sub_schedules} ORDER BY om_sub_schedule_id DESC LIMIT 1`)
        await db.query(`ALTER TABLE ${table.tb_r_om_sub_schedules} ALTER COLUMN om_sub_schedule_id RESTART WITH ${(lastTransSubSchedule.rows[0]?.om_sub_schedule_id ?? 0) + 1}`)

        await db.query(`DELETE FROM ${table.tb_r_om_main_schedules} WHERE created_by = '${flagCreatedBy}'`)
        const lastTransMainSchedule = await db.query(`SELECT *, date(created_dt) as created_date FROM ${table.tb_r_om_main_schedules} ORDER BY om_main_schedule_id DESC LIMIT 1`)
        await db.query(`ALTER TABLE ${table.tb_r_om_main_schedules} ALTER COLUMN om_main_schedule_id RESTART WITH ${(lastTransMainSchedule.rows[0]?.om_main_schedule_id ?? 0) + 1}`)
    }
    //#endregion

    //#region clear master data with flag
    {
        await db.query(`DELETE FROM ${table.tb_m_om_item_check_kanbans} WHERE created_by = '${flagCreatedBy}'`)
        ...
        await db.query(`DELETE FROM ${table.tb_m_machines} WHERE created_by = '${flagCreatedBy}'`)
        ...
        await db.query(`DELETE FROM ${table.tb_m_system} WHERE created_by = '${flagCreatedBy}'`)
        const lastSystem = await db.query(`SELECT *, date(created_dt) as created_date FROM ${table.tb_m_system} ORDER BY system_id DESC LIMIT 1`)
        await db.query(`ALTER TABLE ${table.tb_m_system} ALTER COLUMN system_id RESTART WITH ${(lastSystem.rows[0]?.system_id ?? 0) + 1}`)
    }
    //#endregion
...
const main = async () => {
    ...
    try
    {
        ...
        const json = JSON.parse(await readFile(path.resolve(__dirname, '../json/omCamShaftRedWhite.json'), "utf8"));
        for (let i = 0; i < json.length; i++)
        {
            ...
            const frequency = cleanString(json[i].Frequency)
            ...
            {
                const newlyInsertedItemCheckSql = `insert into ${table.tb_m_om_item_check_kanbans}
                ...
                itemCheckKanbanId = newlyInsertedItemCheck.rows[0].om_item_check_kanban_id
            } else
            {
