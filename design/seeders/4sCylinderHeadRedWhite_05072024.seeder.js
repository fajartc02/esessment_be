...
const flagCreatedBy = 'SEEDER CylinderHead 05072024'
...
const main = async () => {
    ...
    try
    {
        ...
        const lineQuery = await db.query(`select * from tb_m_lines where line_nm in ('Cylinder Head') and deleted_dt is null limit 1`)
        const lineRow = lineQuery.rows[0]

        const json = JSON.parse(await readFile(path.resolve(__dirname, '../json/4sCylinderHeadRedWhite_05072024.json'), "utf8"));
