...
const flagCreatedBy = 'SEEDER 4sMainLine 14042025'
...
const main = async () => {
    ...
    try
    {
        ...
        const lineQuery = await db.query(`select * from tb_m_lines where line_nm in ('Main Line') and deleted_dt is null limit 1`)
        const lineRow = lineQuery.rows[0]

        const json = JSON.parse(await readFile(path.resolve(__dirname, '../json/4sMainLine_14042025.json'), "utf8"));
        for (let i = 0; i < json.length; i++)
        {
            ...
            {
                noKanban = String(json[i]['No Kanban']).replace(/("|')/g, "").trim()
            }
            ...
            //#region item check
            let itemCheckNm = cleanString(json[i]['Item Check'] || json[i]['Item Check Kanban'])
            if (itemCheckNm)
            {
