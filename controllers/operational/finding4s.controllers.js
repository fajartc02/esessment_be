...
module.exports = {
    ...
    try {
      ...
      if (result.length > 0) {
        if (nullId) {
          ...
          result = result[0];
          result.kaizen_file = result.kaizen_file
            ? process.env.APP_HOST + "/file?path=" + result.kaizen_file
            : null;
        }
      }
  ...
  edit4sScoreFinding: async (req, res) => {
    try {
      let finding4sId = await uuidToId(
        table.tb_r_4s_findings,
        "finding_id",
        req.params.id
      );
      ...
      let attrsUpdateUser4s = await attrsUserUpdateData(req, scoreUpdate);
  ...
  upload4sImageFinding: async (req, res) => {
    try {
      ...
      response.success(
        res,
        "Success to upload 4s image finding",
        req.body.finding_img
      );
    } catch (error) {
      response.failed(
        res,
        "Error to upload 4s image finding " + error?.message ?? ""
      );
    }
  },
    ...
    try {
      try {
        ...
        if (!queryFindFinding.length) {
          response.failed(
            res,
            "Error to upload 4s kaizen finding, finding not found"
          );
          return;
        }
        ...
        if (
          !queryFindFinding.item_check_kanban_id &&
          req.body.item_check_kanban_id
        ) {
        ...
        response.failed(
          res,
          "Error to upload 4s kaizen finding " + error?.message ?? ""
        );
      }
    } catch (error) {
      console.log(error);
