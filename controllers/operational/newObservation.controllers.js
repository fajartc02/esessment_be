  ...
  addFindingObsCheck: async (req, res) => {
    try {
      ...
      const transaction = await queryTransaction(async (db) => {
        req.body.category_id = req.body.category_id ? `(select category_id from ${table.tb_m_categories} where uuid = '${req.body.category_id}')` : null
        req.body.factor_id = req.body.factor_id ? `(select factor_id from ${table.tb_m_factors} where uuid = '${req.body.factor_id}')` : null
        req.body.line_id = req.body.line_id ? `(select line_id from ${table.tb_m_lines} where uuid = '${req.body.line_id}')` : null
        req.body.cm_pic_id = req.body.cm_pic_id ? `(select user_id from ${table.tb_m_users} where uuid = '${req.body.cm_pic_id.pic_id}')` : null
        req.body.cm_result_factor_id = req.body.cm_result_factor_id ? `(select factor_id from ${table.tb_m_factors} where uuid = '${req.body.cm_result_factor_id}')` : null
        console.log(req.body);
        ...
        let resultFindingResponse = await queryPostTransaction(db, table.tb_r_result_findings, resultFindingData)
        ...
        return await queryPostTransaction(db, table.tb_r_findings, attrsUserInsert)
        // await queryPOST(table.)
      })
      response.success(res, 'Success to add finding check observation', transaction);
    } catch (error) {
      console.log(error);
